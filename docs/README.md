...
9. [peripheriques-i2c.md](peripheriques-i2c.md) — le bus I2C électrique en drain ouvert : le maître `machine.I2C` cadencé par quarts de période, le décodeur esclave piloté par les fronts (`Internal.PartialI2cDevice`), le contrat des scripts de périphérique (`on_write`/`on_read`/`outputs`/`lines`), les résistances de tirage, et l'écran Grove LCD RGB piloté par un driver du commerce.
