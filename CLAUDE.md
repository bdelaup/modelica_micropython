...
├── Examples/                   -- un modèle par scénario de vérification (BasicBlink, SleepCompression, InputReactivity, ScriptError, PinEcho, AdcRead, PwmLed, ImportDemo, PinIrq, TimerToggle, DisplayDemo, UartLoopback, UartEcho, UartEchoTable, UartSensor, UartRegulation, UartGps, UartStateMachine, UartLcdDemo) + LedChaser (chenillard bidirectionnel, démonstrateur)
└── Resources/
    ...
    ├── Scripts/                -- scripts des exemples (demo.py par défaut, un script par scénario d'Examples/) + _shim/machine_time_shim.py (le shim machine/time lui-même, lu et exécuté par PyRuntime_new avant le script utilisateur — source unique, plus de miroir dans docs/). Un script décrivant un périphérique se nomme `*_device.py` et vit à plat dans ce dossier ; chaque périphérique fourni a le sien par défaut
